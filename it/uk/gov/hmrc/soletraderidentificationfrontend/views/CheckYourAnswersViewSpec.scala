...
class CheckYourAnswersViewSpec extends ComponentSpecHelper with ViewSpec {

  "GET /check-your-answers-business" should {
    lazy val result: WSResponse = get("/check-your-answers-business")
    lazy val doc: Document = Jsoup.parse(result.body)

    "return a view with the correct title" in {
      doc.title mustBe messages.title
    }

    "return a view with the correct heading" in {
      doc.getH1Elements.text mustBe messages.heading
    }

    "return a view with a summary list which" should {
      ...
      "has a first name row" in {
        ...
        firstNameRow.getSummaryListChangeLink mustBe routes.PersonalDetailsController.show().url
        firstNameRow.getSummaryListChangeText mustBe s"${Base.change} ${messages.firstName}"
      }

      "has a last name row" in {
        ...
        lastNameRow.getSummaryListChangeLink mustBe routes.PersonalDetailsController.show().url
        lastNameRow.getSummaryListChangeText mustBe s"${Base.change} ${messages.lastName}"
      }

      "has a date of birth row" in {
        ...
        dateOfBirthRow.getSummaryListChangeLink mustBe routes.PersonalDetailsController.show().url
      ...
      "has a nino row" in {
        ...
        ninoRow.getSummaryListChangeLink mustBe routes.EnterNinoController.show().url
      ...
      "has an sautr row" in {
        ...
        sautrRow.getSummaryListChangeLink mustBe routes.SaUtrController.show().url
        sautrRow.getSummaryListChangeText mustBe s"${Base.change} ${messages.sautr}"
      }

      "return a view with a continue and confirm button" in {
